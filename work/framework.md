...
### 33.服务器架构
    mongodb副本集 一对多个 逻辑服节点  （根据压测结果决定单个副本集对应N个逻辑服节点）
    逻辑服节点 一个对多个 游戏服       （
    网关集集群 一对多
    
    登录服        一对多
    跨服活动服务器 多对多 （）
    跨服聊天服务器 一对多（功能上也会限制操作数据次数，基本上是读取较多，压力基本可以忽略）
    跨服工会服务器 一对多（功能上也会限制操作数据次数，基本上是读取较多，压力基本可以忽略）
...
### 36.登录排队问题
    为什么需要 登陆排队机制
        登陆的时候 需要load db  几十个table  大量的IO，登陆主要慢在IO阻塞这块
    分游戏类型：全球服和单服
    单服：
    只要登录成功，就与网关建立连接，然后游戏节点（无论是按最少分配还是hash分配）人数已满，
    在对应节点进行排队，设置好队列根据登录耗时设计每次处理几个玩家登录。
    
    全球服：网关分配游戏节点策略 人数最少游戏节点，而且会设置单节点最大人数上限
    
### 37.Slg内存问题
    一般内存常驻，即过期时间设置得长一点，加非常多的tick()